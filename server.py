...
class Server(messaging.ServerClientBase):
    MAX_U_NAME_LEN = 16
    MIN_U_NAME_LEN = 4
    ...
    def change_user_name(self, requested_user, new_user_name):
        ...
        if not self.validate_user_name(new_user_name):
            msg = "Username size must be between " + \
        ...
        if name_taken:
            self.send_msg_as_sys_to_user('That name is already taken', requested_user)
            return

        sys_msg = 'User ' + requested_user.name
        requested_user.name = new_user_name
        sys_msg += ' has changed their user name to ' + new_user_name
